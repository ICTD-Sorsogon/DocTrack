...
const actions = {
    ...
    async getArchiveDocuments({ commit }, filter) {
        const response = await axios.post(
            `/api/get_archive_documents`,
            filter.filterBy == "Year"
                ? { selected: filter.year.list, filterBy: "Year" }
                : { selected: filter.date.list, filterBy: "Date" }
        );
    ...
    async createNewDocument({ commit, dispatch }, form) {
        await axios
            .post(`/api/add_new_document/${form.id ?? ""}`, form)
            .then(response => {
                let res = {
                    status: "success",
            ...
            .catch(error => {
                let res = {
                    status: "failed",
                    message: `The server replied with an error! Please Contact your
    ...
    async receiveDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/receive_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async forwardDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/forward_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async terminateDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/terminate_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
                ...
                commit("SNACKBAR_STATUS", data);
                commit("GET_ALL_ARCHIVE_DOCUMENTS", {
                    action: "update",
                    hasNewTerminated: true
                });
            })
            .catch(error => {
                const error_data = {
    ...
    async acknowledgeDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/acknowledge_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async holdDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/hold_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async changeDateDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/change_date_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async releaseDocumentConfirm({ commit }, form) {
        await axios
            .post(`/api/release_document_confirm/${form.id}`, form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async restoreDocument({ commit }, document) {
        let status = {};
        await axios
            .post("/api/restore_document", document)
            .then(response => {
        ...
        if (response.action == "new") {
            ...
            if (response.hasNewTerminated) {
                if (state.documentsArchive.length) {
                    ...
                    newState("newTerminated", {
                        yearFromDb: [new Date().getFullYear()],
                        data: []
                    });
                }
            } else {
                state.documentsArchive[0].year = response.yearFromDb.map(
                    String
                );
                ...
                state.documentsArchive[0].selected[filter].text =
                    response.filterBy;
                state.documentsArchive[0].selected[filter].list =
