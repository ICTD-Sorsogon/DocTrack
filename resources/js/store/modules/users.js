...
function buildName(first_name, middle_name, last_name, suffix) {
    ...
    f_name =
        first_name
            .trim()
            .charAt(0)
            .toUpperCase() + first_name.trim().slice(1);
    m_name =
        middle_name
            .trim()
            .charAt(0)
            .toUpperCase() + middle_name.trim().slice(1);
    l_name =
        last_name
            .trim()
            .charAt(0)
            .toUpperCase() + last_name.trim().slice(1);
    if (suffix != null && typeof suffix !== "undefined") {
        s_name =
            suffix
                .trim()
                .charAt(0)
                .toUpperCase() + suffix.trim().slice(1);
    }
...
const actions = {
    ...
    async getAllUsersComplete({ commit }) {
        await axios.get("/api/all_users_complete").then(response => {
            response.data.forEach(element => {
                element.full_name = "";
                element.full_name = buildName(
                    element.first_name,
    ...
    async updateExistingUser({ commit }, form) {
        await axios
            .post("/api/update_existing_user", form)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async deleteExistingUser({ commit }, id) {
        await axios
            .post(`/api/delete_existing_user/${id}`)
            .then(response => {
                const data = {
                    status: "SUCCESS",
    ...
    async uploadProfilePicture({ commit }, image) {
        ...
        await axios
            .post(`/api/upload_profile_picture`, data, config)
            .then(response => {
                const data = {
                    status: "SUCCESS",
