                ...
                >
                    <template v-slot:top>
                        <v-dialog
                            ...
                            max-width="80vw"
                        >
                            <v-card v-if="final_data.length">
                                <v-card-title>
                                    <span class="headline">{{
                                        formTitle
                                    }}</span>
                                    <v-spacer></v-spacer>
                                    <v-icon @click="close" large class="ml-4">
                                        mdi-close
                                    </v-icon>
                                </v-card-title>
...
export default {
    ...
    methods: {
        ...
        editItem(item) {
            ...
            if (
                item.original_values != null &&
                item.new_values.destination_office != undefined
            ) {
    ...
    computed: {
        ...
        logs() {
            var getAll = JSON.parse(
                JSON.stringify(this.$store.state.users.all_users)
            );
            var new_logs = JSON.parse(
                JSON.stringify(this.$store.state.users.logs)
            );
            if (new_logs.length > 0) {
                new_logs.forEach(log => {
                    ...
                    if (log.original_values) {
                        getAll.forEach(user => {
                            if (user.id == log.original_values.sender_name) {
                                log.original_values.sender_name =
                                    user.last_name +
                                    "," +
                                    user.first_name +
                                    " " +
                    ...
                    if (log.original_values) {
                        this.document_types.forEach(doc_type => {
                            if (
                                doc_type.id == log.original_values.document_type
                            ) {
                                log.original_values.document_type =
                                    doc_type.name;
                            }
                        });
                    }
                    ...
                    if (log.original_values) {
                        this.offices.forEach(office => {
                            if (
                                office.id ==
                                log.original_values.destination_office
                            ) {
                                log.original_values.destination_office =
                                    office.name;
                            }
