...
export default {
    ...
    computed: {
        ...mapState({
            tracking_reports: state => state.documents.tracking_reports
        }),
        ...
        data() {
            ...
            let summary = {};
            let record = this.tracking_reports_data.length
                ? groupBy(this.tracking_reports_data, "transaction_of")
                : false;
            ...
            if (record) {
                for (let i in record) {
                    ...
                    let efficiency =
                        (((transaction - delayed) / transaction) * 100).toFixed(
                            2
                        ) + "%";
                    let average = formatDistanceStrict(
                        0,
                        record[i].reduce((counter, value, index) => {
                            return (
                                (counter * index + value.speed) / (index + 1)
                            );
                        }, 0) * 1000
                    );
            ...
            let officeReport =
                this.auth_user.office.office_code == "DO"
                    ? groupBy(this.office_reports_get, "touched_by")
                    : { [this.auth_user.id]: [this.office_reports_get] };
            for (let n in officeReport) {
                summary[n] = {
                    ...summary[n],
                    ...officeReport[n].reduce(
                        (accumulator, value) => {
                            accumulator.office = {
                                name: offices[n - 1],
                                id: n
        ...
        tracking_reports_data: {
            get: function() {
                return JSON.parse(
                    JSON.stringify(Object.values(this.filterData))
                );
            },
            set: function(val) {
                this.filterData = val;
        ...
        filterTrackingReport() {
            if (this.filterDateFrom != "" && this.filterDateTo != "") {
                ...
                let filter = this.tracking_reports_data.filter(item => {
                    return (
                        new Date(item.last_touched).getTime() >= start &&
