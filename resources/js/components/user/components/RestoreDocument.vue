            ...
            <v-card-text>
                <v-simple-table>
                    ...
                    <template v-slot:default>
                        ...
                        <tbody>
                            <tr
                                v-for="docu in selectedDocument"
                                :key="docu.recipient_id"
                            >
                                <td>{{ docu.office }}</td>
                                <td>
                                    {{
                                        new Date(docu.updated_at)
                                            .toISOString()
                                            .substr(0, 10)
                                    }}
                                </td>
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            <v-text-field
                                ...
                                required
                            />
                            <span v-if="!valid" style="color:red"
                                >Please re-type
                                <strong> {{ confirmOfficeCode }} </strong> to
                                confirm</span
                            >
                            <span v-if="valid" style="color:green"
                                >Confirmation is valid, Please click submit
...
export default {
    ...
    watch: {
        selectedDocument(newVal) {
            try {
                newVal.filter(d => d.deleted_at != null).length ??
    ...
    computed: {
        ...
        selectedDocument() {
            ...
            try {
                var documents = arc[arc.filter.toLowerCase()].data.find(
                    d => d.id == this.document.id
                ).incoming_trashed;
                documents.forEach(document => {
                    var destination = this.offices.find(
                        o => o.id == document.destination_office
                    );
                    if (destination) {
    ...
    methods: {
        restoreDocument() {
            if (this.valid) {
                this.$store
                    .dispatch("restoreDocument", this.restoreParam)
                    .then(() => {
                        if (this.request.status == "success") {
                            this.$store
                                ...
                                .then(() => {
                                    this.$emit(
                                        "reload-data",
                                        this.param.filter == "Year"
                                            ? true
                                            : false,
                                        this.param.filter == "Date"
                                            ? true
                                            : false
                                    );
                                    this.confirmField = "";
