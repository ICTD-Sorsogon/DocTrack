<template>
    <v-row class="d-flex">
        <v-col v-bind="bp(6)">
            <v-text-field
                v-model="advanceSearch.trackingId"
                label="Traking ID"
                class="mx-4"
                clearable
            />
        ...
        <v-col v-bind="bp(6)">
            <v-select
                class="mx-4"
                :items="advanceSearch.optionSource"
                v-model="advanceSearch.source"
                    ...
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-chip
                                ...
                                close
                                @click:close="
                                    removeSelectedChips('source', item)
                                "
                            >
                                {{ item }}
                            </v-chip>
                    ...
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-chip
                                ...
                                close
                                @click:close="
                                    removeSelectedChips('type', item.name)
                                "
                            >
                                {{ item.name }}
                            </v-chip>
                    ...
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-chip
                                ...
                                close
                                @click:close="
                                    removeSelectedChips('destination', item)
                                "
                            >
                                {{ item.office_code || item }}
                            </v-chip>
                    ...
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-chip
                                ...
                                close
                                @click:close="
                                    removeSelectedChips('sender', item)
                                "
                            >
                                {{ item.full_name || item }}
                            </v-chip>
            ...
            >
                ...
                >
                    ...
                    <v-btn
                        text
                        color="primary"
                        @click="advanceSearch.optionDateCreated = false"
                    >
                        Cancel
                    </v-btn>
                    <v-btn
                        ...
                        color="primary"
                        @click="
                            $refs.dialog_created.save(advanceSearch.dateCreated)
                        "
                    >
                        OK
                    </v-btn>
        ...
        <v-col v-bind="bp(12)" align="right">
                ...
                <template v-slot:default="dialog">
                    <v-card color="grey lighten-2">
                        <v-card-title class="headline">
                            <v-icon class="mr-2" size="30px">mdi-cancel</v-icon>
                            Cancel
                        </v-card-title>
                        <v-card-text>
                            Are you sure you want to cancel advanced search it
                            will reset the search parameter!
                        </v-card-text>
                        <v-card-actions>
                            <v-spacer />
                            ...
                            <v-btn text @click.prevent="$emit('changeSearch')">
                                YES
                            </v-btn>
                        </v-card-actions>
                    </v-card>
            ...
            <v-btn
                ...
                class="mb-2 ma-1 ml-0 mr-2 pl-13 pr-13"
                @click.stop="submitSearchParameter"
            >
...
export default {
    ...
    computed: {
        ...
        hasSearchParam() {
            ...
            let watchSearchParam = (trackingId == null
              ? ""
              : trackingId.trim() != "")
                ? true
                : false || (subject == null ? "" : subject.trim() != "")
                ? true
                : false ||
                  source.length > 0 ||
                  type.length > 0 ||
    ...
    methods: {
        removeSelectedChips(el, item) {
            this.advanceSearch[el].splice(
                this.advanceSearch[el].indexOf(item),
                1
            );
    ...
    mounted() {
        ["atype", "aoriginating", "adestination"].forEach(
            b => (this.$refs[b].lastItem = 200)
        );
    }
};
</script>
