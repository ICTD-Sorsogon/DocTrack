<template>
    <v-card flat>
        <v-card-title primary-title>
            ...
            Document : {{ selected_document.tracking_code }}
                    ...
                    <v-row>
                        <v-col cols="12" xl="6" lg="6" md="6">
                            <v-combobox
                                ...
                                >{{
                                    selected_document.originating_office
                                }}</v-combobox
                            >
                        </v-col>
                        <v-col cols="12" xl="6" lg="6" md="6">
                        ...
                        <v-col
                            cols="12"
                            :xl="types == 'forward' ? 4 : 6"
                            :lg="types == 'forward' ? 4 : 6"
                            :md="types == 'forward' ? 4 : 6"
                        >
                            <v-text-field
                        ...
                        <v-col
                            cols="12"
                            xl="12"
                            :lg="types == 'terminal' ? 12 : 6"
                            md="12"
                            v-if="
                                ['forward', 'receive', 'terminal'].includes(
                                    types
                                )
                            "
                        >
            ...
            <v-dialog v-model="documentDialog" persistent max-width="450px">
                <v-card color="grey lighten-2">
                    <v-card-title class="headline">
                        ...
                        >
                        {{
                            $route.params.type.replace(/\w/, val =>
                                val.toUpperCase()
                            )
                        }}
                        Document
                    ...
                    <v-card-text>
                        ...
                        {{ $route.params.type.replace(/\w/, val => val) }} this
                        document? <br />
                        <strong>- {{ selected_document.subject }}</strong>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn
                        ...
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="receiveDocumentConfirm"
                        >
                            Receive
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="forwardDocumentConfirm"
                        >
                            Forward
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="terminateDocumentConfirm"
                        >
                            Terminate
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="acknowledgeDocumentConfirm"
                        >
                            Acknowledge
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="holdDocumentConfirm"
                        >
                            Hold
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="changeDateDocumentConfirm"
                        >
                            Save
                        </v-btn>
                        <v-btn
                            ...
                            color="primary darken-1"
                            text
                            @click.prevent="releaseDocumentConfirm"
                        >
                            Release
                        </v-btn>
                    </v-card-actions>
...
export default {
    ...
    computed: {
        ...
        selected_document() {
            let userDoc =
                !this.is_admin &&
                ["terminate", "Change Date"].includes(this.$route.params.type)
                    ? this.documents["outgoing"]
                    : this.documents["incoming"];

            return JSON.parse(
                JSON.stringify(this.is_admin ? this.documents : userDoc)
            )
                .map(doc => {
        ...
        offices_to() {
            return this.$store.state.offices.offices.filter(
    ...
    methods: {
        ...
        sanitize() {
            ...
            this.form.recipient_id = this.is_admin
                ? null
                : this.form.document_recipient[0].recipient_id;
        ...
        receiveDocumentConfirm() {
            ...
            this.$store
                .dispatch("receiveDocumentConfirm", this.form)
                .then(() => {
        ...
        forwardDocumentConfirm() {
            ...
            !this.sent && this.sanitize();
            this.$store
                .dispatch("forwardDocumentConfirm", this.form)
                .then(() => {
                    if (this.request.status == "SUCCESS") {
                        this.$store
                            .dispatch("setSnackbar", {
        ...
        terminateDocumentConfirm() {
            this.btnloading = true;
            this.$store
                .dispatch("terminateDocumentConfirm", this.form)
                .then(() => {
                    if (this.request.status == "SUCCESS") {
                        this.$store
                            .dispatch("setSnackbar", {
        ...
        acknowledgeDocumentConfirm() {
            this.btnloading = true;
            this.$store
                .dispatch("acknowledgeDocumentConfirm", this.form)
                .then(() => {
                    if (this.request.status == "SUCCESS") {
                        this.$store
                            .dispatch("setSnackbar", {
        ...
        changeDateDocumentConfirm() {
            this.btnloading = true;
            this.$store
                .dispatch("changeDateDocumentConfirm", this.form)
                .then(() => {
                    if (this.request.status == "SUCCESS") {
                        this.$store
                            .dispatch("setSnackbar", {
        ...
        releaseDocumentConfirm() {
            this.btnloading = true;
            this.$store
                .dispatch("releaseDocumentConfirm", this.form)
                .then(() => {
                    if (this.request.status == "SUCCESS") {
                        this.$store
                            .dispatch("setSnackbar", {
